...
Window_BattleStatus.prototype.drawGaugeArea = function(rect, actor) {
    ...
    this.drawActorHp2(actor, rect.x, wy - wymod, rect.width);
    if (this.getGaugesDrawn(actor) <= 2) {
      this.drawActorMp2(actor, rect.x, wy, rect.width);
    } else {
      var ww = rect.width / 2;
      this.drawActorMp2(actor, rect.x, wy, rect.width);
      this.drawActorTp2(actor, rect.x + ww, wy, ww);
    }
...
Window_BattleStatus.prototype.drawStatusFace = function(index) {
    ...
    var wy = rect.y + 4;
    this.drawActorFace2(actor, wx, wy, ww, wh);
};
...
//=============================================================================
// Nomaldog New Customize
//=============================================================================
Window_Base.prototype.drawActorHp2 = function(actor, x, y, width) {
    width = width || 186;
    this.changePaintOpacity(255);
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.hpA, x+65, y-5, 44);
    this.drawCurrentAndMax(actor.hp, actor.mhp, x, y-5, width,
                           this.hpColor2(actor), this.normalColor());
};

Window_Base.prototype.drawActorMp2 = function(actor, x, y, width) {
    width = width || 186;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.mpA, x+65, y, 44);
    this.drawCurrentAndMax(actor.mp, actor.mmp, x, y, width,
                           this.mpColor2(actor), this.normalColor());
};
Window_Base.prototype.hpColor2 = function(actor) {
    var ratio = actor.hp/actor.mhp;
    var color;
    if(ratio > 0.5){
       color =  '#FFFFFF';
    }else if(ratio > 0.25){
...
Window_Base.prototype.mpColor2 = function(actor) {
    return '#FFFFFF';
};
Window_Base.prototype.drawActorFace2 = function(actor, x, y, width, height) {
    var regex1 = /<face1\s(\d+)>/;
    var regex2 = /<face2\s(\d+)>/;
    var regex3 = /<face3\s(\d+)>/;
    var note   = $dataActors[actor._actorId].note;
    var res;
    var faceName = actor.faceName();
    var faceIndex = actor.faceIndex();
    var ratio = actor.hp/actor.mhp;
    if(ratio <= 0.25){
        while (res = regex3.exec(note)) {
            faceName = $dataActors[res[1]].faceName;
            faceIndex = $dataActors[res[1]].faceIndex;
            break;
        }
    }else if(ratio <= 0.5){
        while (res = regex2.exec(note)) {
            faceName = $dataActors[res[1]].faceName;
            faceIndex = $dataActors[res[1]].faceIndex;
            break;
        }
    }else{
        while (res = regex1.exec(note)) {
            faceName = $dataActors[res[1]].faceName;
            faceIndex = $dataActors[res[1]].faceIndex;
            break;
        }
